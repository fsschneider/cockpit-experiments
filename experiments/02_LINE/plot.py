...
def read_data(path):
    ...
    losses = plotter.tracking_data.train_loss.dropna().to_numpy()
    traj = np.concatenate(plotter.tracking_data.Parameters.dropna().to_numpy(), axis=0)
...
def plot_loss(ax, data):
    ...
    ax.set_ylabel("Mini-Batch Train Loss")
    ax.tick_params(axis="both", which="both", length=0)
...
if __name__ == "__main__":
    ...
    basepath = os.path.join(
        ...
        "two_d_quadratic",
        "SGD",
        "num_epochs__20__batch_size__128__l2_reg__0.e+00__lr__1.e+00__"
        + "momentum__0.e+00__nesterov__False",
    )
    # This refers so specific files.
    # Running 02_run_Line.py will produce them, but they will be called
    # slightly different
    large_path = "random_seed__42__2021-05-10-14-44-21__log"
    short_path = "random_seed__42__2021-05-10-14-44-31__log"
    ...
    fig, axs = plt.subplots(
        1,
        3,
        figsize=_get_plot_size(textwidth="neurips", height_ratio=0.9, subplots=(1, 3)),
    )
    ...
    # Save plot
    plt.savefig(os.path.join(SAVEDIR, "LINE.pdf"), bbox_inches="tight")
    plt.show()

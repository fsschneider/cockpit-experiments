...
EXP_DIR = os.path.dirname(os.path.dirname(HERE))


def fix_deepobs_data_dir(path=None):
    """Fix DeepOBS' data directory to one path avoid multiple dataset copies."""
    if path is None:
        # First check whether there is a data_deepobs dir in the experiments dir
        if os.path.exists(os.path.join(EXP_DIR, "data_deepobs")):
            path = os.path.join(EXP_DIR, "data_deepobs/")
        else:
            path = "~/tmp/data_deepobs"
    set_data_dir(os.path.expanduser(path))
